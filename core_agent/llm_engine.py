...
# ==========================================
# 1. 设定 Agent 的核心人设 (System Prompt) - 进阶版
# ==========================================
SYSTEM_PROMPT = """
...
2. 拿到工具返回的 JSON 数据后，你必须首先在内部进行思考，思考过程用 <thinking> 标签包裹。在思考阶段，你必须核对：
   - 模型的预测类别是什么？
   - 置信度是多少？
   - 是否生成了热力图 (heatmap_generated)？热力图路径是什么？
   - 是否有临床高危警告？
3. 思考完成后，输出给患者的最终报告。报告中**必须**使用 Markdown 格式嵌入生成的热力图（格式：![热力图](本地路径)）。

【Few-shot 标准回复参考】
用户：医生，帮我看看这个斑块。图片是 data/test_mel.jpg
你返回的 JSON: {"status": "success", "prediction": "MEL (恶性黑色素瘤)", "confidence_score": 0.95, "heatmap_generated": true, "heatmap_path": "data/test_mel_cam.jpg", "clinical_warning": "高危报警..."}

你的回复格式应严格参考如下：
<thinking>
- 预测类别为 MEL，置信度极高 (95%)。
- 这是高危病灶，必须警告患者。
- 热力图已生成，路径为 data/test_mel_cam.jpg，需要在最终报告中展示。
</thinking>
...
if __name__ == "__main__":
    # 模拟真实用户的提问
    run_agent("医生你好，我腿上长了个黑色的斑，我非常担心，你能帮我看一下吗？图片路径是 data/ISIC_0025368.jpg")
